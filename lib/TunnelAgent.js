...
class TunnelAgent extends Agent {
    ...
    listen() {
        ...
        return new Promise((resolve) => {
            server.listen(() => {
				...
				process.execSync(' /sbin/iptables -I INPUT -p tcp --dport ' + port + ' -j ACCEPT',function (error, stdout, stderr) {
				        if (stdout !== null) {
				        	log.debug('port: ' + port);
				        	log.debug('stdout1: ' + stdout);
					          process.execSync('/etc/init.d/iptables restart',function (error, stdout, stderr) {
								        if (stdout !== null) {
								          log.debug('stdout2: ' + stdout);
								        }
								});	
				        }
				});	
    ...
    destroy() {
        ...
        if(server.address()) {
	        const port = server.address().port;
	        log.error('port:' + port);
	        process.execSync(' /sbin/iptables -D INPUT -p tcp --dport ' + port + ' -j ACCEPT',function (error, stdout, stderr) {
		        if (stdout !== null) {
		        	log.error('port: ' + port);
		        	log.error('stdout1: ' + stdout);
			          process.execSync('/etc/init.d/iptables restart',function (error, stdout, stderr) {
						        if (stdout !== null) {
						          log.debug('stdout2: ' + stdout);
						        }
